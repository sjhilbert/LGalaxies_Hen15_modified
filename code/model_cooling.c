...
/* metallicies with repect to solar converted to
   absolut metallicity by adding  log10(Z_sun) = -1,69897 for Zsun=0.02 */
...
void compute_cooling(const int galaxy_number_, const double dt_)
{
  ...
  double coolingGas;
  if(Gal[galaxy_number_].HotGas > 1.0e-6)
  {
    ...
    const double HotRadius = (Gal[galaxy_number_].Type == 0) ? Gal[galaxy_number_].Rvir : Gal[galaxy_number_].HotRadius;
...
/** @brief calculates the energy released by black holes due to passive accretion,
  ...
  */
void do_AGN_heating(double dt_, int ngal)
{
  ...
  int galaxy_number_;
  for (galaxy_number_ = 0; galaxy_number_ < ngal; galaxy_number_++)
  {
    ...
    if(HotGas > 0.0)
    {
      if(AGNRadioModeModel == 0)
      {
        AGNrate = AgnEfficiency * UnitTime_in_s * Gal[galaxy_number_].BlackHoleMass/Hubble_h * (HotGas/Hubble_h) *
...
//                       * ((HotGas / HotRadius * Rvir / Mvir) / 0.1);
        ...
        AGNrate = AgnEfficiency * UnitTime_in_s * Gal[galaxy_number_].BlackHoleMass * pow3(Vvir) * HotGas * Rvir / (HotRadius * Mvir) *
      ...
      else if(AGNRadioModeModel == 3 || AGNRadioModeModel == 4)
      {
        ...
        { AGNrate = (2.5 * M_PI * G) * (0.75 * 0.6 * x) * Gal[galaxy_number_].BlackHoleMass * 0.15; }
      ...
      if(AGNheating > Gal[galaxy_number_].CoolingGas)
      { AGNheating = Gal[galaxy_number_].CoolingGas; }
    ...
    if(Gal[galaxy_number_].CoolingGas < 0.0)
...
void cool_gas_onto_galaxy(int galaxy_number_, double dt_)
{
  ...
  double Mdisk=Gal[galaxy_number_].ColdGas;
  double Mcool=Gal[galaxy_number_].CoolingGas;

  if (Mcool>Gal[galaxy_number_].HotGas)
  { Mcool = Gal[galaxy_number_].HotGas; }
  ...
  if(Mcool > 0.0)
  {
    ...
    double fraction=((float)Mcool)/Gal[galaxy_number_].HotGas;
    ...
    if (DiskRadiusModel == 0)
    {
      ...
      { for (i=0;i<3;i++) { Gal[galaxy_number_].GasSpin[i]=(Gal[galaxy_number_].GasSpin[i]*Mdisk+Gal[galaxy_number_].HaloSpin[i]*Mcool)/(Gal[galaxy_number_].ColdGas); }  }
